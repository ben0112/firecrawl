...
async function getJob(id, _logger = logger_1.logger) {
    const [nuqJob, dbScrape, gcsJob] = await Promise.all([
        nuq_router_1.scrapeQueue.getJob(id, _logger),
...
async function getJobs(ids, _logger = logger_1.logger) {
    const [nuqJobs, dbScrapes, gcsJobs] = await Promise.all([
        nuq_router_1.scrapeQueue.getJobs(ids, _logger),
...
async function crawlStatusController(req, res, isBatch = false) {
    ...
    let warning;
    try {
        const robotsBlocked = await redis_1.redisEvictConnection.smembers("crawl:" + req.params.jobId + ":robots_blocked");
        ...
        logger_1.logger.debug("Failed to check robots blocked URLs", {
            error,
            zeroDataRetention,
        });
    }
    ...
    if (!warning && currentStatus !== "scraping" && resultCount <= 1) {
        ...
        const crawl = await (0, crawl_redis_1.getCrawl)(req.params.jobId);
        if (crawl && crawl.originUrl && !(0, url_utils_1.isBaseDomain)(crawl.originUrl)) {
            // Don't show warning if user is already using crawlEntireDomain
            const isUsingCrawlEntireDomain = crawl.crawlerOptions?.crawlEntireDomain === true;
            if (!isUsingCrawlEntireDomain) {
                const baseDomain = (0, url_utils_1.extractBaseDomain)(crawl.originUrl);
                if (baseDomain) {
    ...
    const createdAtMs = sc?.createdAt;
    const lastDoneMs = status !== "scraping"
        ? await (0, crawl_redis_1.getLastDoneJobTimestamp)(req.params.jobId)
        : null;
    const completedAtMs = status === "completed" || status === "failed" || status === "cancelled"
